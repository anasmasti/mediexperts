  ...
  <div>
    ...
    <div style="width:100%;" class="hide-from-print">
      ...
      <select
        ...
        @change="handleAction('G6/FetchReferencesPlan', selected_nrc_entrp)"
      ...
      <div style="width:100%;" class="hide-from-print">
        ...
        <select
          ...
          style="width:100%; padding: .5rem; border: 1px solid #000;"
          @change="
            handleAction('G6/FetchActionByReference', id_plan),
    ...
    <div v-for="(info, index) in G6_info" :key="index">
      <div class="rows">
        <div>
          <label for="entrepriseDirecteur" style="font-size:15px"
            >Je soussigné (Nom du représentant légal de l’entreprise) :</label
          >
          <span style="background-color:yellow"> {{ info.nom_dg1 }} </span>
        </div>
        <div style="margin-top:5px">
          <label for="raison_social" style="font-size:15px">
            Raison Social :</label
          >
        ...
        <div style="margin-top:5px">
          <label for="year" style="font-size:15px">
            ...
            <span style="background-color:yellow">{{ info.annee }}</span> Se
            présente comme suit :
          </label>
        </div>
        <div style="margin-top:5px">
          <label style="font-size:15px"
            >Nombre des actions réalisées (1) =
          </label>
          <span style="background-color:yellow"> {{ nombreActionReas }} </span>
        </div>
        <div style="margin-top:5px">
          <label style="font-size:15px"
            >Nombre total des actions prévues (2) =
          </label>
          <span style="background-color:yellow">
            {{ actions_by_plan.length }}
          </span>
        </div>
        <div style="margin-top:5px">
          <label style="font-size:15px"
            >Taux de réalisation (1) / (2) (**) =</label
          >
          <span style="background-color:yellow">
            {{ nombreActionReas == 0 ? (tauxReas = 0) : tauxReas }} %</span
          >
        </div>
      </div>
      ...
      <div class="table" style="margin-top:10px">
        <table>
          ...
          <tr
            class="center"
            v-for="(action, index) in actions_by_plan"
            :key="index"
          >
      ...
      <div class="d-flex" style="margin-left:45%;margin-top:35px;">
        <label for="Directeur">
          ...
          <label style="margin-top:10px;"
            ...
            Cachet de l’entreprise et signature</label
          >
        </label>
      </div>
    </div>
    ...
    CalculNombreActionRealise() {
      setTimeout(() => {
        ...
        taux_realisation = Math.round(
          parseFloat((counter / total_prevus) * 100).toPrecision(4)
        );
